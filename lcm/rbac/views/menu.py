...
def  menu_list(request):
    ...
    if menus:
        default_mid = models.Menu.objects.first().id
        # default_sid = default_mid
        default_sid = models.Permission.objects.filter(menu_id=default_mid).first().id
    else:
        default_mid = None
        default_sid = None
    
    menu_id = request.GET.get('mid', default_mid)  # 用户选择的一级菜单
    second_menu_id = request.GET.get('sid', default_sid)  # 用户选择的二级菜单
